...
static unsigned g_ginsengVreg = 0;
static bool g_bGinsengValidVreg = false;
bool llvm::ginsengIsSSDATA(unsigned vreg, bool bPrintErr) {
  if (!g_bGinsengValidVreg) {
    if (bPrintErr) llvm::errs() << "NO SSDATA IS SET\n";
    return false;
  }

  return (vreg == g_ginsengVreg);
}

void llvm::ginsengSetSSDATA(unsigned vreg) {
  g_ginsengVreg = vreg;
  g_bGinsengValidVreg = true;
}

// This is same as errs() except for it prints out my header
raw_ostream &llvm::ymh_log() {
  static raw_fd_ostream S(STDERR_FILENO, false, true);
  S << "##YMH## ";
  return S;
}

// the folllowing function is copied from...
// stacktrace.h (c) 2008, Timo Bingmann from http://idlebox.net/
// published under the WTFPL v2.0
// just rename the func
/** Print a demangled stack backtrace of the caller function to FILE* out. */
// static inline void print_stacktrace()
void llvm::ymh_printStack()
{
